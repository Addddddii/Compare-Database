...
@Slf4j
@FXMLController
public class SqlServerConfigLeftController implements Initializable {
    ...
    private void databaseChange(){
        ...
        if (StringUtils.isNotBlank(this.database.getValue())) {
            List<String> schemaNameList = databaseStructureLoader.loadSchemaByDataBaseName(Constant.LEFT, this.database.getValue(), DataBaseEnum.SqlServer.getType());
    ...
    private void schemaChange(){
        MainController.leftSchemaName = this.schema.getValue();
    }
